...
TELEGRAM_TOKEN = os.getenv("TELEGRAM_TOKEN", "8193806217:AAFSU9SjPFhtC8-ZluLpPUGoLBz9mpAKDEE")
ADMIN_CHAT_IDS = os.getenv("ADMIN_CHAT_IDS", "7162158003,7446601898,7349777538").split(",")
DB_FILE = "keys.json"
db_lock = Lock()
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "create_package":
        ...
        keys_to_delete = [k for k, d in API_KEYS.items() if k not in ["packages", "global_banned_devices"] and isinstance(d, dict) and d.get("package") == pkg_name]
...
async def ban_device_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"Device ID banned:\n“{dev_id}”")
...
async def unban_device_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if was_banned:
        await update.message.reply_text(f"Device ID unbanned:\n“{dev_id}”")
    else:
...
app = FastAPI(title="ZOLDIK API", lifespan=lifespan)

@app.get("/validate")
async def validate(api_key: str = Query(...), device_id: str = Query(...)):
    ...
    nice_message = (
        f"Welcome to Zoldik!\n\n"
        f"Package: {key_data.get('package', 'Standard')}\n"
...
async def generate_udid_profile(api_key: str = Query(...)):
    ...
    plist_dict = {
        ...
        "PayloadDescription": "ZOLDIK UDID Registration - Used for device activation",
        "PayloadDisplayName": "ZOLDIK Device Registration",
        "PayloadIdentifier": f"com.zoldik.udid.{profile_uuid}",
        "PayloadOrganization": "ZOLDIK",
    ...
    return Response(
        ...
        headers={"Content-Disposition": 'attachment; filename="zoldik-udid.mobileconfig"'}
    )
...
if __name__ == "__main__":
    import uvicorn
    print("Starting ZOLDIK API server on port 8000 ...")
